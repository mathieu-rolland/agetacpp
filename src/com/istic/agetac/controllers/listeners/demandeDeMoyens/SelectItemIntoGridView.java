...
/**
* Classe ListenerMoyens de DemandeDeMoyensFragment : Permet de changer le thème de l'item que 
* l'on sélectionne dans le gridView des moyens.  
*
* @author Anthony LE MEE - 10003134
*/
public class SelectItemIntoGridView implements OnClickListener {
	...
	/**
	 * Construteur
	 * @param a DemandeDeMoyensFragment de DemandeDeMoyensFragment
	 * @param position int de l'item
	 * @param s DemandeMoyensSavedInstanceState de DemandeDeMoyensFragment
	 * @param listMoyen GridView des moyens
	 */
	public SelectItemIntoGridView(DemandeDeMoyensFragment a, int position)  
	{  
			...
			if (v != null) {
				...
				for (int i = 0 ; i < this.demandeDeMoyens.getGridViewMoyens().getCount(); i++) {
					...
					if (i != position) 
					{
						...
						((Button) this.demandeDeMoyens.getGridViewMoyens().getChildAt(i)).setBackgroundColor(Color.parseColor(new String("#293133")));
	
					}
					else
					{
						// Sinon je lance la surbrillance
						((Button) this.demandeDeMoyens.getGridViewMoyens().getChildAt(i)).setBackgroundColor(Color.parseColor(new String("#efefef")));
